...
public class CamphorLeavesBlock extends ModLeavesBlock {
    public CamphorLeavesBlock(Properties properties) {
        super(properties);
    }
    ...
    public void animateTick(BlockState state, Level world, BlockPos pos, RandomSource random) {
        ...
        if (random.nextInt(260) ==0) {
            ...
            if (!isFaceFull(blockState.getCollisionShape(world, blockPos), Direction.UP)) {
                ParticleUtils.spawnParticleBelow(world, pos, random, ModParticleTypes.CAMPHOR_LEAVES.get());
            }
        }
    }
}
