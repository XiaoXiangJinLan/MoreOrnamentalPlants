...
public class CamelliaLeavesBlock extends ModLeavesBlock {
    public CamelliaLeavesBlock(Properties properties) {
        super(properties);
    }
    ...
    public void animateTick(BlockState state, Level world, BlockPos pos, RandomSource random) {
        ...
        if (random.nextInt(160) ==0) {
            ...
            if (!isFaceFull(blockState.getCollisionShape(world, blockPos), Direction.UP)) {
                ParticleUtils.spawnParticleBelow(world, pos, random, ModParticleTypes.CAMELLIA_LEAVES.get());
            }
        }
    }
}
