...
public class WhitePeachLeavesBlock extends ModLeavesBlock {
    public WhitePeachLeavesBlock(Properties properties) {
        super(properties);
    }
    ...
    public void animateTick(@NotNull BlockState state, @NotNull Level world, @NotNull BlockPos pos, @NotNull RandomSource random) {
        ...
        if (random.nextInt(15) ==0) {
            ...
            if (!isFaceFull(blockState.getCollisionShape(world, blockPos), Direction.UP)) {
                ParticleUtils.spawnParticleBelow(world, pos, random, ModParticleTypes.WHITE_PEACH_LEAVES.get());
            }
        }
    }
}
