...
public class FlavescensMeiLeavesBlock extends ModLeavesBlock {
    public FlavescensMeiLeavesBlock(Properties properties) {
        super(properties);
    }
    ...
    public void animateTick(@NotNull BlockState state, @NotNull Level world, @NotNull BlockPos pos, @NotNull RandomSource random) {
        ...
        if (random.nextInt(20) ==0) {
            ...
            if (!isFaceFull(blockState.getCollisionShape(world, blockPos), Direction.UP)) {
                ParticleUtils.spawnParticleBelow(world, pos, random, ModParticleTypes.FLAVESCENS_MEI_LEAVES.get());
            }
        }
    }
}
