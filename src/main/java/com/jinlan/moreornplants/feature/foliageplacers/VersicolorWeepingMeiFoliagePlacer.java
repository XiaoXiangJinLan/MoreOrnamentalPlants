...
public class VersicolorWeepingMeiFoliagePlacer extends FoliagePlacer {
    public static final Codec<VersicolorWeepingMeiFoliagePlacer> CODEC = RecordCodecBuilder.create(instance ->
            ...
            )).apply(instance, VersicolorWeepingMeiFoliagePlacer::new)
    );
    ...
    public VersicolorWeepingMeiFoliagePlacer(IntProvider radius, IntProvider offset, IntProvider height, float wideBottomLayerHoleChance, float cornerHoleChance, float hangingLeavesChance, float hangingLeavesExtensionChance) {
        super(radius, offset);
        this.height = height;
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.VERSICOLOR_WEEPING_MEI_FOLIAGE_PLACER.get();
    }
    ...
    private void generateSingleVine(LevelSimulatedReader level, FoliageSetter foliageSetter, RandomSource random, BlockPos startPos) {
        ...
        for (int i = 0; i < vineLength - 1; i++) {
            ...
            foliageSetter.set(plantPos, vineType.plantBlock.defaultBlockState());
        }

        // 放置藤蔓头部在最下方，并设置年龄为25阻止自然生长
        BlockState headState = vineType.headBlock.defaultBlockState()
                .setValue(vineType.ageProperty, 25);
    ...
    private VineType getRandomVineType(RandomSource random) {
        ...
        if (chance < 0.5f) { // 50% 概率生成复色
            return VineType.VERSICOLOR;
        } else if (chance < 0.75f) { // 25% 概率生成粉色 (50% + 25% = 75%)
            return VineType.PINK;
        } else { // 25% 概率生成白色 (75% + 25% = 100%)
    ...
    private enum VineType {
        ...
        public final net.minecraft.world.level.block.state.properties.IntegerProperty ageProperty;

        VineType(net.minecraft.world.level.block.Block plantBlock, net.minecraft.world.level.block.Block headBlock, net.minecraft.world.level.block.state.properties.IntegerProperty ageProperty) {
