...
public class SweetgumFoliagePlacer extends FoliagePlacer {
    public static final Codec<SweetgumFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> foliagePlacerParts(instance).and(
            IntProvider.codec(0, 24).fieldOf("trunk_height").forGetter((placer) -> placer.trunkHeight)
    ).apply(instance, SweetgumFoliagePlacer::new));

    private final IntProvider trunkHeight;

    public SweetgumFoliagePlacer(IntProvider radius, IntProvider offset, IntProvider trunkHeight) {
        super(radius, offset);
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.SWEETGUM_FOLIAGE_PLACER.get();
    }
    ...
    protected void createFoliage(LevelSimulatedReader level, FoliageSetter blockSetter, RandomSource random,
        ...
        // 中间层 - 中层（固定三层，半径2）
        int middleLayers = 4; // 固定三层
        for (int i = 0; i < middleLayers; i++) {
            createMiddleLayer(level, blockSetter, random, config, center, offset - 3 - i, attachment.doubleTrunk(), i);
        }

        int middleLayerBottom = offset - 3 - middleLayers;

        // 中下层 - 新增一层（半径2，只跳过四个角）
        createAdditionalLowerMiddleLayer(level, blockSetter, random, config, center, middleLayerBottom, attachment.doubleTrunk());

        // 中下层 - 原来两层（半径3）
        createLowerMiddleLayer(level, blockSetter, random, config, center, middleLayerBottom - 1, attachment.doubleTrunk(), true);
        createLowerMiddleLayer(level, blockSetter, random, config, center, middleLayerBottom - 2, attachment.doubleTrunk(), false);

        // 最底层 - 半径2（比银杏的半径1加一格），不做特殊处理
        this.placeLeavesRow(level, blockSetter, random, config, center, 2, middleLayerBottom - 3, attachment.doubleTrunk());
    }
    ...
    private void createMiddleLayer(LevelSimulatedReader level, FoliageSetter blockSetter, RandomSource random,
                                   TreeConfiguration config, BlockPos center, int y, boolean doubleTrunk, int layerIndex) {
        // 半径2，跳过四个角，最边缘随机跳过
        for (int x = -2; x <= 2; x++) {
            for (int z = -2; z <= 2; z++) {
    ...
    // 新增的中下层第一层（半径2，只跳过四个角）
    private void createAdditionalLowerMiddleLayer(LevelSimulatedReader level, FoliageSetter blockSetter, RandomSource random,
                                                  TreeConfiguration config, BlockPos center, int y, boolean doubleTrunk) {
        // 半径2，只跳过四个角
        for (int x = -2; x <= 2; x++) {
            for (int z = -2; z <= 2; z++) {
            ...
            for (int z = -3; z <= 3; z++) {
                ...
                if (isUpper) {
                    ...
                    if (dist == 3) {
                        ...
                        if (x == 0 && Math.abs(z) == 3) {
                            // 南北方向
                            this.placeLeavesRow(level, blockSetter, random, config, center.offset(0, 0, z > 0 ? 4 : -4), 0, y, doubleTrunk);
                        } else if (z == 0 && Math.abs(x) == 3) {
                            // 东西方向
                            this.placeLeavesRow(level, blockSetter, random, config, center.offset(x > 0 ? 4 : -4, 0, 0), 0, y, doubleTrunk);
                        }
                    }
                }
