...
public class WhiteWeepingMeiFoliagePlacer extends FoliagePlacer {
    public static final Codec<WhiteWeepingMeiFoliagePlacer> CODEC = RecordCodecBuilder.create(instance ->
            ...
            )).apply(instance, WhiteWeepingMeiFoliagePlacer::new)
    );
    ...
    public WhiteWeepingMeiFoliagePlacer(IntProvider radius, IntProvider offset, IntProvider height, float wideBottomLayerHoleChance, float cornerHoleChance, float hangingLeavesChance, float hangingLeavesExtensionChance) {
        super(radius, offset);
        this.height = height;
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.WHITE_WEEPING_MEI_FOLIAGE_PLACER.get();
    }
    ...
    private void generateSingleVine(LevelSimulatedReader level, FoliageSetter foliageSetter, RandomSource random, BlockPos startPos) {
        ...
        for (int i = 0; i < vineLength - 1; i++) {
            ...
            foliageSetter.set(plantPos, ModBlocks.WHITE_WEEPING_MEI_PLANT.get().defaultBlockState());
        }

        // 放置藤蔓头部在最下方
        BlockState headState = ModBlocks.WHITE_WEEPING_MEI.get().defaultBlockState()
                .setValue(WhiteWeepingMeiBlock.AGE, 25); // 设置年龄为25，阻止自然生长
