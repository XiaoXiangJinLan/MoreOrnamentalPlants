...
public class PinkWeepingMeiFoliagePlacer extends FoliagePlacer {
    public static final Codec<PinkWeepingMeiFoliagePlacer> CODEC = RecordCodecBuilder.create(instance ->
            ...
            )).apply(instance, PinkWeepingMeiFoliagePlacer::new)
    );
    ...
    public PinkWeepingMeiFoliagePlacer(IntProvider radius, IntProvider offset, IntProvider height, float wideBottomLayerHoleChance, float cornerHoleChance, float hangingLeavesChance, float hangingLeavesExtensionChance) {
        super(radius, offset);
        this.height = height;
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.PINK_WEEPING_MEI_FOLIAGE_PLACER.get();
    }
    ...
    private void generateSingleVine(LevelSimulatedReader level, FoliageSetter foliageSetter, RandomSource random, BlockPos startPos) {
        ...
        // 放置藤蔓身体部分 (PINK_WEEPING_MEI_PLANT)
        for (int i = 0; i < vineLength - 1; i++) {
            BlockPos plantPos = startPos.below(i);
            foliageSetter.set(plantPos, ModBlocks.PINK_WEEPING_MEI_PLANT.get().defaultBlockState());
        }

        // 放置藤蔓头部 (PINK_WEEPING_MEI) 在最下方
        BlockState headState = ModBlocks.PINK_WEEPING_MEI.get().defaultBlockState()
                .setValue(PinkWeepingMeiBlock.AGE, 25); // 设置年龄为25，阻止自然生长
