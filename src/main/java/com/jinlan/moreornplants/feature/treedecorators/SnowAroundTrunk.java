...
public class SnowAroundTrunk extends TreeDecorator {
    public static final MapCodec<SnowAroundTrunk> CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(
    ...
    private BlockPos findTrunkBase(List<BlockPos> logPositions) {
        ...
        BlockPos lowestPos = logPositions.getFirst();
    ...
    // 在树干周围生成雪层（考虑垂直范围）
    private void generateSnowAroundTrunk(Context context, LevelSimulatedReader level,
    ...
    // 在垂直范围内尝试放置雪层
    private void tryPlaceSnowInVerticalRange(Context context, LevelSimulatedReader level,
    ...
    // 尝试在指定位置放置雪层
    private boolean tryPlaceSnowLayer(Context context, LevelSimulatedReader level,
