...
public class ZiyingFox extends TamableAnimal {
    ...
    protected void registerGoals() {
        ...
        this.panicGoalWild = new ZiyingFoxPanicGoal(2.2);
        this.panicGoalTamed = new ZiyingFoxTamedPanicGoal(1.5, DamageTypeTags.PANIC_ENVIRONMENTAL_CAUSES);
        ...
        this.goalSelector.addGoal(9, new ZiyingFoxStrollThroughVillageGoal(32, 200));
    ...
    public @NotNull InteractionResult mobInteract(Player player, @NotNull InteractionHand hand) {
        ...
        if (this.level().isClientSide) {
            boolean flag = this.isOwnedBy(player) || this.isTame() || itemstack.is(ModItems.GOLDEN_CRABAPPLE) && !this.isTame();
            ...
            } else if (itemstack.is(ModItems.GOLDEN_CRABAPPLE) && !this.isTame()) {
                itemstack.consume(1, player);
    ...
    public Predicate<ItemStack> getTemptItem() {
        return stack -> stack.is(ModTags.Items.ZIYING_FOX_APPEAL_TO);
    }

    public boolean isHoldingItemAppealTo(LivingEntity entity) {
        if (entity instanceof Player player) {
            return this.getTemptItem().test(player.getMainHandItem()) ||
                    this.getTemptItem().test(player.getOffhandItem());
        }
        return false;
    }
    ...
    protected void reassessTameGoals() {
        ...
        if (this.panicGoalWild != null) {
            this.goalSelector.removeGoal(this.panicGoalWild);
        }
        if (this.panicGoalTamed != null) {
            this.goalSelector.removeGoal(this.panicGoalTamed);
        }
        if (this.isTame()) {
            if (this.panicGoalTamed != null) {
                this.goalSelector.addGoal(1, this.panicGoalTamed);
            }
        } else {
            if (this.panicGoalWild != null) {
                this.goalSelector.addGoal(1, this.panicGoalWild);
            }
        }
    }
    ...
    public boolean isAlertableCached() {
        if (this.alertableCooldown-- <= 0) {
            this.alertableCooldown = 10; // 每 0.5 秒刷新一次
    ...
    public class ZiyingFoxEatBerriesGoal extends MoveToBlockGoal {
        ...
        protected void onReachedTarget() {
            if (EventHooks.canEntityGrief(ZiyingFox.this.level(), ZiyingFox.this)) {
    ...
    class ZiyingFoxPanicGoal extends PanicGoal {
        public ZiyingFoxPanicGoal(double speedModifier) {
            super(ZiyingFox.this, speedModifier);
        }

        @Override
        public boolean shouldPanic() {
            return !ZiyingFox.this.isDefending() && super.shouldPanic();
        }
    }

    class ZiyingFoxTamedPanicGoal extends TamableAnimalPanicGoal {
        public ZiyingFoxTamedPanicGoal(double speedModifier, TagKey<DamageType> panicCausingDamageTypes) {
    ...
    class ZiyingFoxSearchForItemsGoal extends Goal {
        ...
        public void tick() {
            ...
            if (itemstack.isEmpty() && !list.isEmpty()) {
                ZiyingFox.this.getNavigation().moveTo(list.getFirst(), 1.2F);
            }
        }
    ...
    class ZiyingFoxStrollThroughVillageGoal extends StrollThroughVillageGoal {
        public ZiyingFoxStrollThroughVillageGoal(int unused32, int interval) {
            super(ZiyingFox.this, interval);
        }
